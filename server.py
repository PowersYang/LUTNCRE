...
def render_charts(date):
    ...
    c1 = pass_rate_bar(date, analysis.pass_rate('college'), '各学院通过率', 500, 300)
    c2 = pass_rate_bar(date, analysis.pass_rate('major'), '各专业通过率', 500, 300)
    c3 = pass_rate_bar(date, analysis.pass_rate('subject'), '各科目通过率', 500, 300)
    c4 = pie_rich_label(analysis.pass_rate_fd(), '人数统计', 500, 300)
    c5 = gaugegauge_base_color(analysis.pass_rate(), '整体通过率', 500, 300)
    c6 = geo_effectscatter(analysis.pass_rate('jg'), '各省通过率', 500, 300)
